...
## Simetria Composta ($\text{CS}$):

Pressuposto: variância constante ($\sigma^2$) e correlação constante ($\rho$).

$$\begin{array}{c|c}
\begin{aligned}
\text{Matriz de Covariância }: \mathbf{\Sigma}_i^{\text{CS}} = \sigma^2 \left( \mathbf{I} (1-\rho) + \mathbf{J} \rho \right) \\ \\
\text{Forma Matricial:} \quad \mathbf{\Sigma}_i^{\text{CS}} =
\begin{pmatrix}
\sigma^2 & \sigma^2 \rho & \dots \\
\sigma^2 \rho & \sigma^2 & \dots \\
\vdots & \vdots & \ddots
\end{pmatrix} \\ \\
...
\text{Forma Matricial:} \quad \mathbf{R}_i^{\text{CS}} =
\begin{pmatrix}
1 & \rho & \dots \\
\rho & 1 & \dots \\
\vdots & \vdots & \ddots
...
$$\begin{array}{c|c} 
\begin{aligned} 
\text{Matriz de Covariância }: \mathbf{\Sigma}_i^{\text{AR}(1)} = \sigma^2 \mathbf{R}_i(\rho) \\ \\ \text{Forma Matricial:} \quad \mathbf{\Sigma}_i^{\text{AR}(1)} = \begin{pmatrix} 
\sigma^2 & \sigma^2 \rho^1 & \dots \\ \sigma^2 \rho^1 & \sigma^2 & \dots \\ \vdots & \vdots & \ddots \end{pmatrix} \\ \\ \text{Tal que} \quad \left[\mathbf{\Sigma}_i^{\text{AR}(1)}\right]_{jk} = \sigma^2 \rho^{|t_{ij} - t_{ik}|} \end{aligned} \quad \quad & \quad \quad \begin{aligned} \text{Matriz de Correlação }: \mathbf{R}_i^{\text{AR}(1)} = \mathbf{R}_i(\rho) \\ \\ 
\text{Forma Matricial:} \quad \mathbf{R}_i^{\text{AR}(1)} = \begin{pmatrix} 1 & \rho^1 & \dots \\ \rho^1 & 1 & \dots \\ \vdots & \vdots & \ddots \end{pmatrix} 
\\ \\ \text{Tal que} \quad \left[\mathbf{R}_i^{\text{AR}(1)}\right]_{jk} = \rho^{|t_{ij} - t_{ik}|} \end{cases} \end{aligned} \end{array}$$

Pressuposto: O modelo estima livremente todas as variâncias ($\sigma_j^2$) e covariâncias ($\sigma_{jk}$).

$$\begin{array}{c|c}
\begin{aligned}
\text{Matriz de Covariância }: \mathbf{\Sigma}_i^{\text{SYMM}} = \begin{pmatrix} \sigma_1^2 & \sigma_{12} & \dots \\ \sigma_{21} & \sigma_2^2 & \dots \\ \vdots & \vdots & \ddots \end{pmatrix} \\ \\
\text{Forma Matricial:} \quad \mathbf{\Sigma}_i^{\text{SYMM}} =
\begin{pmatrix}
\sigma_1^2 & \sigma_{12} & \dots \\
\sigma_{21} & \sigma_2^2 & \dots \\
\vdots & \vdots & \ddots
\end{pmatrix} \\ \\
...
\quad \quad & \quad \quad
\begin{aligned}
\text{Matriz de Correlação }: \mathbf{R}_i^{\text{SYMM}} = \begin{pmatrix} 1 & \rho_{12} & \dots \\ \rho_{21} & 1 & \dots \\ \vdots & \vdots & \ddots \end{pmatrix} \\ \\
\text{Forma Matricial:} \quad \mathbf{R}_i^{\text{SYMM}} =
\begin{pmatrix}
1 & \rho_{12} & \dots \\
\rho_{21} & 1 & \dots \\
\vdots & \vdots & \ddots
