...
#| code-summary: "Comparação de Modelos (AIC, BIC e LRT)"

# Comparação de modelos pelo Critério AIC
aic_result <- stats::AIC(mod_cs, mod_ar1, mod_symm)
print("Resultados do AIC para os modelos:")
print(aic_result)

# Comparação de modelos pelo Critério BIC
bic_result <- stats::BIC(mod_cs, mod_ar1, mod_symm)
print("Resultados do BIC para os modelos:")
print(bic_result)

# Teste da Razão de Verossimilhança (LRT) entre modelos aninhados
print("Teste de Razão de Verossimilhança (LRT) entre mod_cs e mod_ar1:")
print(stats::anova(mod_cs, mod_ar1))

print("Teste de Razão de Verossimilhança (LRT) entre mod_ar1 e mod_symm:")
print(stats::anova(mod_ar1, mod_symm))

print("Teste de Razão de Verossimilhança (LRT) entre mod_cs e mod_symm:")
...
$$
\text{Pontuacao}_{it} = \beta_0 + \beta_1 \;\text{Grupo}_{i} + \beta_2 \;\text{Semana Recentralizada}_{t} + \beta_3 \; (\text{Grupo}_i \times \text{Semana Recentralizada}_t) + \varepsilon_{it}, 
\quad 
\boldsymbol{\varepsilon} \sim N(\mathbf{0}, \boldsymbol{\Sigma}_{AR(1)})
$$
...
# Extrair estimativas do modelo AR(1)
tidy(mod_ar1)
```
