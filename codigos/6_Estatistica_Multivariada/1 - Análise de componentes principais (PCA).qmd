---
title: "1. Análise de Componentes Principais (PCA)"
# description: "Uma descrição curta e impactante."
author: "Alexssandro Oliveira | alexssandrosg@gmail.com"
lang: pt
date: last-modified
# image: "imagens/modulo_inferencia.jpg"  # <--- IMPORTANTE: Salve um print do gráfico aqui com esse nome!
categories: ["R", "Python", "1 - PCA"] # <--- Suas tags para o filtro
---
...
A Análise de Componentes Principais - PCA é uma técnica estatística que visa explicar a estrutura de variância e covariância de um vetor aleatório $\bm{X}$. Esse método se baseia na criação de combinações lineares a partir das variáveis originais, denominadas componentes principais, de modo que são não correlacionadas. O objetivo prático dessa abordagem é simplificar o conjunto de dados, reduzindo o número de variáveis. Essa simplificação é alcançada ao identificar novas componentes que expliquem a maior parte da variabilidade nos dados.


A priori, devemos entender como aplicar o método, que pode ser feito de duas formas:

-   **1 Método (Matriz de Correlação):** Devemos obter a matriz de correlação $R_{m \times n}$ dos dados originais, que é quivalente a obter a matriz de covariância $S_{m \times n}$ a partir dos dados transformados "padronizados".


-   **2 Método (Matriz de Covariância::):** Devemos obter a matriz de covariância $S_{m \times n}$ a partir dos dados originais.

Uma vez obtida a matriz de interesse, primeiramente extraímos os autovalores e autovetores associados a ela. A interpretação dos resultados é simples: as combinações lineares (componentes) geradas são interpretadas como sendo a transposta da matriz de autovetores,  iremos denotadar por $P_{m \times n}$, onde cada componente captura uma parte da variabilidade total dos dados. Os autovalores, denominados por $\lambda_{1}, \lambda_{2},..., \lambda_{n}$, representam a variância individual de cada componente e indicam a importância relativa da mesma na explicação da variabilidade total dos dados. 
